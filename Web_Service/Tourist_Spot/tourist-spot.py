...
class UserHandler(tornado.web.RequestHandler, Database):
    ...
    async def post(self):
        ...
        try:
            ...
            mEntryFeeAd = self.request.arguments.get('entry fee for adult')
            ...
            mEntryFeeCh = self.request.arguments.get('entry fee for child')
            ...
            mVisitingHours = self.request.arguments.get('visiting hours')


            # Validation
            if not mVisitingHours:
                code = 4041
                message = 'Visitings Hours are required'
                raise Exception

            visiting_hours_pattern = r'^\d{2} (AM|PM) - \d{2} (AM|PM)$'
            if not re.match(visiting_hours_pattern, mVisitingHours):
                code = 4042
                message = 'Visiting Hours must be in "HH AM/PM - HH AM/PM" format'
